...
export const projectHandler = {
    async create(name: string, sourcePath?: string) {
    ...
    async configRuntime(models: string[]) {
        const pm = new ProjectManagerImpl();
        const cs = new ConfigServiceImpl();
        const config = await cs.getConfig();
        if (!config.activeProjectId) return console.log(chalk.red("Error: No active project selected."));

        const project = await pm.getProject(config.activeProjectId);
        if (!project) return console.log(chalk.red(`Error: Project '${config.activeProjectId}' not found.`));
    ...
    async configPrediction(enabled: string) {
        const pm = new ProjectManagerImpl();
        const cs = new ConfigServiceImpl();
        const config = await cs.getConfig();
        if (!config.activeProjectId) return console.log(chalk.red("Error: No active project selected."));
    ...
    async context() {
        ...
        const context: ProjectContext = { project, skills, prompts };
...
export const promptHandler = {
    async add(text: string) {
        ...
        const config = await cs.getConfig();
        if (!config.activeProjectId) return console.log(chalk.red("No active project."));
    ...
    async list() {
        ...
        const config = await cs.getConfig();
        if (!config.activeProjectId) return console.log("No active project.");
...
export const warmHandler = {
    async warm() {
        ...
        await orchestrator.execute(plan, event => {
            switch (event.type) {
                case "ModelLoading": console.log(chalk.dim(`Loading model: ${event.model}...`)); break;
                case "ModelLoaded": console.log(chalk.green(`Model loaded: ${event.model}`)); break;
                case "PrimingPrompt": console.log(chalk.dim(`Priming prompt: ${event.prompt}...`)); break;
                case "PromptCompleted": console.log(chalk.green(`Prompt primed.`)); break;
                case "Error": console.log(chalk.red(`Error: ${event.message}`)); break;
                case "Completed": console.log(chalk.bold.green("Warmup Completed.")); break;
            }
        });
    }
};
